...
test_start "devops tool chain integration"
	...
	lazy_command_pattern "^git push.*github\.com" "GITHUB_TOKEN"
	lazy_command_pattern "^git push.*gitlab\.com" "GITLAB_TOKEN"
...
test_start "ci/cd pipeline simulation"
	...
	lazy_command_pattern "^terraform apply" "DEPLOY_TOKEN"
	lazy_command_pattern "^kubectl apply" "DEPLOY_TOKEN"
...
test_start "1password integration simulation"
	...
	lazy_command_pattern "^git (push|pull|fetch).*" "GITHUB_TOKEN"
	lazy_command_pattern "^aws .*" "AWS_ACCESS_KEY"
...
test_start "docker and kubernetes workflow"
	...
	lazy_command_pattern "^kubectl .*" "KUBERNETES_CONFIG"
	lazy_command_pattern "^helm .*" "HELM_REPOSITORY_PASSWORD,HELM_NAMESPACE"
...
test_start "complex pattern matching performance"
	...
	for i in {1..10}; do
		...
		lazy_command_pattern "pattern_$i.*complex.*regex.*test" "PATTERN_VAR_$i"
	done
