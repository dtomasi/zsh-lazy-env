...
test_start "lazy_list_vars shows global variables"
	...
	assert_contains "$output" "pending" "Should show pending status"
...
test_start "lazy_list_commands shows pattern command mappings"
	test_setup
	lazy_var "GIT_VAR" "echo 'git-value'"
	lazy_command_pattern "^git push.*github" "GIT_VAR"
...
test_start "lazy_list_commands truncates long patterns"
	...
	lazy_command_pattern "^very_long_command_pattern_that_exceeds_column_width_and_should_be_truncated" "LONG_VAR"
...
test_start "lazy_list_directories shows pattern directory mappings"
	test_setup
	lazy_var "PATTERN_VAR" "echo 'pattern-value'" "pattern:.*/terraform/.*"
	lazy_directory_pattern ".*/client-.*" "CLIENT_VAR"
